...
public class CustomerMapActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private GoogleMap mMap;
    Location mLastLocation;//every 1000 sec it will get new lat and lng
    LocationRequest mLocationRequest;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //here customer logout activity from the firebase
        mLogout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                FirebaseAuth.getInstance().signOut();
        ...
        mRequest.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (requestBol){
                    ...
                    requestBol = true;
                    //getting the customer id from the firebase
                    String userId = FirebaseAuth.getInstance().getCurrentUser().getUid();

                    //when customer willdo request it will update the location
                    //by geofire
                    DatabaseReference ref = FirebaseDatabase.getInstance().getReference("customerRequest");
                    GeoFire geoFire = new GeoFire(ref);
                    geoFire.setLocation(userId, new GeoLocation(mLastLocation.getLatitude(), mLastLocation.getLongitude()));

                    //creating pickup maker for the location
                    pickupLocation = new LatLng(mLastLocation.getLatitude(), mLastLocation.getLongitude());
                    pickupMarker = mMap.addMarker(new MarkerOptions().position(pickupLocation).title("Pickup Here").icon(BitmapDescriptorFactory.fromResource(R.mipmap.ic_pickup)));
    ...
    */
/**
     * This function will get the most clostes driver for the customer
     *//*
